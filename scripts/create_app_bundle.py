#!/usr/bin/env python3
"""
Create macOS Application Bundle for Requirements to Test
...
def create_app_bundle():
	...
	# - Include the source package so we can import `requirements_to_test`
	root = Path.cwd()
	src_dir = root / "src"
	if src_dir.exists():
		# Copy the entire src tree (contains `requirements_to_test` package)
	...
	# No database required for Requirements to Test
...
# Ensure we can import the `requirements_to_test` package from Resources/src
...
try:
    ...
    from requirements_to_test.gui import main as run_main
    run_main()
...
"""
	...
	plist_data = {
		...
		'CFBundleIdentifier': 'com.requirements_to_test.app',
		'CFBundleVersion': '1.0.0',
		'CFBundleShortVersionString': '1.0.0',
		'CFBundleExecutable': app_name.replace(" ", ""),
		'CFBundleIconFile': 'requirements_to_test.icns',
...
def create_icns_icon(resources_path):
	...
	try:
		...
		iconset_dir = resources_path / "requirements_to_test.iconset"
		iconset_dir.mkdir(exist_ok=True)

		# Icon size mappings for macOS
		icon_mappings = {
			'icon_16x16.png': 'requirements_to_test_16x16.png',
			'icon_16x16@2x.png': 'requirements_to_test_32x32.png',
			'icon_32x32.png': 'requirements_to_test_32x32.png',
			'icon_32x32@2x.png': 'requirements_to_test_64x64.png',
			'icon_128x128.png': 'requirements_to_test_128x128.png',
			'icon_128x128@2x.png': 'requirements_to_test_256x256.png',
			'icon_256x256.png': 'requirements_to_test_256x256.png',
			'icon_256x256@2x.png': 'requirements_to_test_512x512.png',
			'icon_512x512.png': 'requirements_to_test_512x512.png',
			'icon_512x512@2x.png': 'requirements_to_test_1024x1024.png'
		}
		...
		icns_path = resources_path / "requirements_to_test.icns"
		...
		if result.returncode == 0:
			...
			largest_icon = icons_dir / "requirements_to_test_512x512.png"
			if largest_icon.exists():
				shutil.copy2(largest_icon, resources_path / "requirements_to_test.icns")

	except Exception as e:
		print(f"⚠️  Could not create ICNS icon: {e}")
		# Copy any available icon as fallback
		if (icons_dir / "requirements_to_test.png").exists():
			shutil.copy2(icons_dir / "requirements_to_test.png", resources_path / "requirements_to_test.icns")
